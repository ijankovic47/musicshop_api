...
public class TypeController {
	...
	public ResponseEntity<?> create(@RequestBody Type type) throws URISyntaxException {
	...
	public ResponseEntity<?> readAll() {
		...
		return ResponseEntity.ok(typeService.readAll());
	}

	@RequestMapping(value = "/{id}", method = RequestMethod.GET)
	public ResponseEntity<?> readById(@PathVariable("id") Integer id) throws NoSuchEntityException {
		
		Optional<Type> type= typeService.readById(id);
		if (type.isPresent()) {
			return ResponseEntity.ok(type.get());
		}
		throw new NoSuchEntityException("Notype found for id = " + id);
	}
}
