...
public class BrandDaoImpl extends GenericDaoImpl<Brand, Integer> implements BrandDao{
	...
	public List<Brand> read(Integer familyId, Integer typeId) {
		...
		Query q=sessionFactory.getCurrentSession().createNativeQuery("select b.id, b.name, b.image, count(i.id) as instrumentCount "
				...
				+ (typeId!=null?"left join type t on t.id=i.type_id where t.id="+typeId:"")
				+ (familyId!=null&&typeId==null?"left join type t on t.id=i.type_id where t.family_id="+familyId:"")
